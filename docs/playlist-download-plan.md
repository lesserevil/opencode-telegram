...
## 11. Implementation Steps (TODO)

- [ ] **Step 1**: Update `youtube.types.ts` with new interfaces
- [ ] **Step 2**: Add playlist detection methods to `youtube.service.ts`
  - `isPlaylistUrl()`
  - `extractPlaylistId()`
- [ ] **Step 3**: Add playlist info retrieval to `youtube.service.ts`
  - `getPlaylistInfo()`
- [ ] **Step 4**: Add sequential download logic to `youtube.service.ts`
  - `downloadPlaylist()`
  - `delay()`
- [ ] **Step 5**: Update `youtube.bot.ts` to handle playlists
